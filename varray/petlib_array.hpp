...
template < typename T > struct varray : public array_base< varray<T>, T> {
    ...
    constexpr varray<T> operator()(const varray<bool>& mask) const noexcept {
        size_type nt = 0;
        for ( auto b : in mask) {
            if (b) ++nt;
        }
        if (nt==0) return varray<T>();
        varray<T> v(nt);
        size_type curr(0);
    ...
    // 
    //
    constexpr varray<T> operator()(const varray<size_type>& ind) const noexcept {
        size_t max_i = ind.max(); 
        varray<T> v(max_i+1,T(0));
        for (size_t i=0;i<sz;++i) {
            v[ind[i]]=data_[i];
        } 
        return v;
