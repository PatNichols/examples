...
"@(#)fppridnt.f90	1.11	15/01/27 Michel Olagnon"
      character (len=*), parameter :: zvers = &
"@(#) fppridnt.f90	V-1.4 15/01/27 Michel Olagnon"
...
!   Macros
      integer, parameter  :: nargm = 64 ! Max # of arguments
...
!
      character (len=*), parameter  :: zobcm = "FPPR_BLANK_CMT"
      integer, save                 :: iwbcm
      ...
      integer, save                 :: iwids
      character (len=*), parameter  :: zoidp = "FPPR_PGM_INDENT"
      integer, save                 :: iwidp
...
! ____________________________________________________________________
      ...
      character (len=2*linem+3), save :: zlinb ! advance line in buffer
      character (len=2*linem+3), dimension (nnstim), save :: zlinbh
      ...
      integer, save     :: nndtu =  3 ! Program unit additional indentation
...
! ____________________________________________________________________
      ...
      use fpprcurs
      interface
         subroutine addtok (ztok, ltok, kktok)
...
! ____________________________________________________________________
!
      write (luerr, "(a)") "This is f90ppr: " // zvers
      write (luerr, "(a)")  zusg
!
...
!
      ...
      do
         if (iffxd == 0) then
            call lexlin (addtok, ifstp, ksta)
         else
            call lexfxd (addtok, ifstp, ksta)
         endif
         if (ksta /= 0) exit
      enddo
!
...
!
         llin = len_trim (zlin)
         if (llin == 0) then
            call trttok (zblk, len(zblk), kkcmt)
            if (ifcnt == 0) then
               exit body
            else
               cycle body
...
!
      ...
      if ((kktok == kkpvg .or. kktok == kkeos) .and.  &
          (ntok > 0)                                ) then
         call prsstt (ntok, ksstt)
         if (ifskp == 0) then
            call outstt (ntok, ksstt)
         endif
         ntok = 0
         itokf = 0
      endif
...
!
      ifdos = 0
      ifdoe = 0
      if (kktokt (itoks) == kknui) then
         itokd = itokdt (itoks)
         itokf = itokft (itoks)
         if ((itokf - itokd + 1) <= llabdm) then
            kktokt (itoks) = kklab
            kctok = kcblb
            itoks = itoks + 1
            ndoe  = 0
            do ilab = nlabd, 1, -1
               if (ztoki (itokd:itokf) == &
                   trim (zlabdt (ilab) (1:llabdm))) then
                  ndoe  = ndoe + 1
                  ifdoe = 1
                  zlabdt (ilab) (1:llabdm) = zlabdt (nlabd) (1:llabdm)
                  nlabd = nlabd - 1
               else
                  exit
               endif
...
!
         if (itoks + 1 < ntok) then
            if (kktokt (itoks+1) == kkdpt .and. &
                kktokt (itoks)   == kkidf) then
               kktokt (itoks) = kkbnm
               kctok = kcbnb
               itoks = itoks + 2
            endif
         ...
         if (kktokt (itok) == kkidf) then
            ...
            if (itok < ntok) then
               ...
               case (kwprc, kwfct)
                  kctok = kcprc
               case (kwass)
                  if (kktokt (itok+1) == kknui) kktokt (itok+1) = kklab
                  kctok = kcass
               case (kwfmt)
                  kctok = kcwtf
               case (kwaio)
               ...
               case (kwgto)
                  if (kktokt (itok+1) == kknui) kktokt (itok+1) = kklab
                  if (kctok == kcbeg .or. kctok == kcbex .or. &
                      kctok == kcblb .or. kctok == kcbnb) then
                     kctok = kcwtx
                  endif
               case default
                  if (kctok == kcbeg .or. kctok == kcbex .or. &
...
!
               if ((ksstt == kswhs .or. ksstt == ksfrs) .and. &
                   (kktokt (itok+1) /= kkcmt .and.            &
                    kktokt (itok+1) /= kkebc)) then
                  ksstt = ksukn
               endif
            endif
!
!  Labels and DO loops
!
         elseif (kktokt (itok) == kkaff) then
            if (ksstt == ksdos .and. ifdos == 0) then
               ifdos = 1
            endif
         elseif (kktokt (itok) == kknui) then
            if (ksstt == ksdos .and. ifdos == 0) then
               ifdos = 1
               itokd = itokdt (itok)
               itokf = itokft (itok)
               if ((itokf - itokd + 1) <= llabdm) then
                  kktokt (itok) = kklab
                  if (nlabd >= nlabdm) then
                     call fpperr ("DO loops too deeply nested")
                  else
                     nlabd = nlabd + 1
                     zlabdt (nlabd) (1:llabdm) = ztoki (itokd:itokf)
                  endif
               endif
            endif
         endif
      enddo
      if (ifdoe /= 0) ksstt = ksdoe
      return
end subroutine prsstt
...
!
      ...
      tnamt (  651) = namtyp (    0,    0,  263,  1231,  1245,  1250,  1264)
      ...
      tnamt ( 3061) = namtyp (    0,    0,  263,  1246,  1259,  1265,  1278)
      ...
      tnamt ( 3501) = namtyp (    0,    0,  263,  1216,  1230,  1235,  1249)
      ...
      tnamt ( 3561) = namtyp (    0,    0,  263,  1201,  1215,  1220,  1234)
...
!
                  elseif (inam == iwidp) then
                     ...
                     if (kerr /= 0) exit body
                     jval = nint (oxptt(1)%dval)
                     if (jval < -nndtp .or. jval > linem/4 ) then
                        call fpperr ("Not an allowed program unit indentation")
                        exit body
                     endif
                     nndtu = jval
!
