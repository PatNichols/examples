...
inline void Chol_i_decomp(int nr, T *__restrict__ a, int lda) {
  ...
  for (register int i = 0; i < nr; p += str, i++) {
    if (i) trsv<lower, notrans, nonunit, T>(i, a, lda, a + i * lda, 1);
...
inline void Chol_i_block_decomp(int nr, T *__restrict__ a, int lda) {
  const int Nb = 40;
  if (nr < Nb) return Chol_i_decomp<T>(nr, a, lda);
...
inline void Chol_k_block_decomp(int nr, T *__restrict__ a, int lda) {
  const int Nb = 40;
  if (nr < Nb) return Chol_k_decomp<T>(nr, a, lda);
...
template <typename T>
inline void Chol_j_block_decomp(int nr, T *a, int lda) {
  const int BZ = 40;

  if (nr < BZ) return Chol_j_decomp<T>(nr, a, lda);
...
class Cholesky_decomposition {
 public:
  template <class A>
  Cholesky_decomposition(const petlib::matrix_base<A, T> &arg);
...
template <typename T>
inline petlib::matrix<T> Cholesky_decomposition<T>::L() const {
...
template <typename T>
inline petlib::matrix<T> Cholesky_decomposition<T>::matrix_solve(
    const petlib::matrix<T> &b) const {
  petlib::matrix<T> x(b);
...
template <typename T>
inline petlib::array<T> Cholesky_decomposition<T>::solve(
    const petlib::array<T> &b) const {
  petlib::array<T> x(b);
