...
dst_iterator elim(src_iterator src, const src_iterator last, src_iterator dst,
                  T x) {
  while (src != last) {
    *dst = *src - x * (*dst);
    ++dst;
...
dst_iterator apply_un_op(src_iterator src, const src_iterator last,
                         dst_iterator dst, unary_op op) {
  while (src != last) {
    *dst = unary_op(*src);
    ++dst;
    ++src;
...
template <class lhs_iterator, class rhs_iterator, class dst_iterator,
          class binary_op>
dst_iterator apply_bin_op(lhs_iterator lhs, const lhs_iterator lhs,
                          rhs_iterator rhs, dst_iterator dst, binary_op op) {
  while (src != last) {
...
iter1 rotate(iter1_t iter, iter1_t last, iter2_t iter2,
             typename iter1_t::value_type angle) {
...
struct slice_iterator {
  ...
  constexpr reference operator[](size_type n) noexcept { return p[n * str]; };

  constexpr slice_iterator& operator++() const noexcept {
    p += str;
    return *this;
  }
  constexpr slice_iterator& operator--() const noexcept {
...
template <typename T>
constexpr bool operator==(const slice_iterator<T>& x,
                          const slice_iterator<T>& y) {
...
std::ostream& print_array =
    [](iter_t iter, iter_t last, std::ostream& os) {
      os << " ( " << *iter;
      while (iter != last) {
        os << ", ";
        os << *iter;
...
struct array_binary_xpr {
  ...
  constexpr value_t operator[](size_type i) const noexcept {
...
struct array_base {
  ...
  constexpr const array_t* leaf() { return static_cast<const array_t*>(this); }
...
#define PETLIB_MAKE_OP_(name_, sym_)                                           \
  ...
  constexpr array_xpr<                                                         \
      array_binary_xpr<name_##Op<T>, array_ref<a_t, T>, array_ref<b_t, T> >,   \
      T>                                                                       \
  ...
  template <typename T, class a_t, class b_t>                                  \
  constexpr array_xpr<                                                         \
      array_binary_xpr<name_##Op<T>, array_ref<a_t, T>, array_xpr<b_t, T> >,   \
      T>                                                                       \
  operator sym_(const array_base<a_t, T>& a, const array_xpr<b_t, T>& b) {     \
    typedef array_binary_xpr<name_##Op<T>, array_ref<a_t, T>,                  \
                             array_xpr<b_t, T> >                               \
        xpr_t;                                                                 \
  ...
  template <typename T, class a_t, class b_t>                                  \
  constexpr array_xpr<                                                         \
      array_binary_xpr<name_##Op<T>, array_xpr<a_t, T>, array_ref<b_t, T> >,   \
      T>                                                                       \
  ...
  template <typename T, class a_t, class b_t>                                  \
  constexpr array_xpr<                                                         \
      array_binary_xpr<name_##Op<T>, array_xpr<a_t, T>, array_xpr<b_t, T> >,   \
      T>                                                                       \
  operator sym_(const array_xpr<a_t, T>& a, const array_xpr<b_t, T>& b) {      \
    typedef array_binary_xpr<name_##Op<T>, array_xpr<a_t, T>,                  \
                             array_xpr<b_t, T> >                               \
        xpr_t;                                                                 \
  ...
  operator sym_(const array_base<a_t, T>& a, const T b) {                      \
    typedef xpr_t                                                              \
        array_binary_xpr<name_##Op<T>, array_ref<a_t, T>, scalar_ref<T> >;     \
    \   
  ...
  operator sym_(const T b, const array_base<a_t, T>& a) {                      \
    typedef xpr_t                                                              \
        array_binary_xpr<name_##RevOp<T>, array_ref<a_t, T>, scalar_ref<T> >;  \
    array_xpr<xpr_t, T>(xpr_t(array_ref<a_t, T>(a), scalar_ref<T>(b)));        \
  }                                                                            \
                                                                               \
  template <typename T, class a_t>                                             \
  ...
  operator sym_(const array_xpr<a_t, T>& a, const T b) {                       \
    typedef xpr_t                                                              \
        array_binary_xpr<name_##Op<T>, array_xpr<a_t, T>, scalar_ref<T> >;     \
  ...
  operator sym_(const T b, const array_xpr<a_t, T>& a) {                       \
    typedef xpr_t                                                              \
        array_binary_xpr<name_##RevOp<T>, array_xpr<a_t, T>, scalar_ref<T> >;  \
    array_xpr<xpr_t, T>(xpr_t(a, scalar_ref<T>(b)));                           \
  }                                                                            \
                                                                               \
  template <typename T, class a_t, class b_t>                                  \
