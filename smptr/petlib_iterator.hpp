...
struct slice_iterator {
  ...
  slice_iterator(pointer p, size_type stride_ = size_type(1))
      : ptr_(p), str_(stride_) {}
  ...
  slice_iterator(const slice_iterator<T>& iter)
      : ptr_(iter.ptr_), str_(iter.str_) {}
  ...
  constexpr slice_iterator<T>& operator++() noexcept {
    ptr_ += str_;
    return *this;
  }
  constexpr slice_iterator<T>& operator--() noexcept {
...
template <typename T>
constexpr slice_iterator<T> operator+(const slice_iterator<T>& iter,
                                      std::ptrdiff_t n) noexcept {
  return slice_iterator<T>(iter.base() + n * iter.stride(), iter.stride());
}

template <typename T>
constexpr slice_iterator<T> operator+(std::ptrdiff_t n,
                                      const slice_iterator<T>& iter) noexcept {
  return slice_iterator<T>(iter.base() + n * iter.stride(), iter.stride());
}
...
template <typename T>
constexpr slice_iterator<T> operator+(std::size_t n,
                                      const slice_iterator<T>& iter) noexcept {
  return slice_iterator<T>(iter.base() + n * iter.stride(), iter.stride());
}

template <typename T>
constexpr std::ptrdiff_t operator-(const slice_iterator<T>& a,
                                   const slice_iterator<T>& b) noexcept {
  return std::ptrdiff_t(a.base() - b.base()) /
         std::ptrdiff_t(a.stride() * sizeof(T));
}

template <typename T>
constexpr bool operator==(const slice_iterator<T>& a,
                          const slice_iterator<T>& b) noexcept {
...
struct array_iterator {
  ...
  constexpr array_iterator<T>& operator++() noexcept {
    ++ptr_;
    return *this;
  }
  constexpr array_iterator<T>& operator--() noexcept {
...
template <typename T>
constexpr array_iterator<T> operator+(const array_iterator<T>& iter,
                                      std::ptrdiff_t n) noexcept {
  return array_iterator<T>(iter.base() + n, iter.stride());
}

template <typename T>
constexpr array_iterator<T> operator+(std::ptrdiff_t n,
                                      const array_iterator<T>& iter) noexcept {
  return array_iterator<T>(iter.base() + n, iter.stride());
}

template <typename T>
constexpr std::ptrdiff_t operator-(const array_iterator<T>& a,
                                   const array_iterator<T>& b) noexcept {
  return std::ptrdiff_t(a.base() - b.base());
}

template <typename T>
constexpr bool operator==(const array_iterator<T>& a,
                          const array_iterator<T>& b) noexcept {
  return (a.base() == b.base());
}
...
#define PETLIB_MAKE_OP(name_, sym_)                          \
  template <class src_iter_t, class dst_iter_t>              \
  dst_iter_t name_##Eq(dst_iter_t dst_iter, dst_iter_t last, \
                       src_iter_t src_iter) {                \
    while (dst_iter != last) {                               \
...
template <typename T>
constexpr bool abs_cmp(T x, T y) noexcept {
  return abs<T>(x) < abs<T>(y);
}

template <class iter_t>
constexpr typename iter_t::value_type amax_element(iter_t iter,
                                                   const iter_t last) noexcept {
...
template <class iter_t>
constexpr typename iter_t::value_type amin_element(iter_t iter,
                                                   const iter_t last) noexcept {
  typedef typename iter_t::value_type value_type;
  value_type cmin(0);
  while (iter != last) {
...
template <class iter_t>
constexpr typename iter_t::value_type max_element(iter_t iter,
                                                  const iter_t last) noexcept {
  typedef typename iter_t::value_type value_type;
...
template <class iter_t>
constexpr typename iter_t::value_type min_element(iter_t iter,
                                                  const iter_t last) noexcept {
  typedef typename iter_t::value_type value_type;
...
constexpr l_iter_t rotate(l_iter_t x, l_iter_t last, r_iter_t y,
                          T angle) noexcept {
...
template <class l_iter_t, class r_iter_t>
constexpr auto dot = [](l_iter_t x, l_iter_t xlast, r_iter_t y) noexcept {
  typedef typename l_iter_t::value_type value_type;
  value_type sum(0);
  while (x != xlast) {
    sum += *x * *y;
...
#define make_op(name_, sym_)                                  \
  constexpr auto name_##OpLam = [](auto x, auto y) noexcept { \
    return x sym_ y;                                          \
  };

make_op(Add, +) make_op(Sub, -) make_op(Mul, *) make_op(Div, /) make_op(Xor, ^)
    make_op(Ior, |) make_op(And, &) make_op(Lsh, <<) make_op(Rsh, >>)
#undef make_op

#define make_op(name_, sym_) \
  constexpr auto name_##OpLam = [](auto x) noexcept { return sym_ x; };

        make_op(Com, ~) make_op(Neg, -) make_op(Pos, +)
#undef make_op

/*
#define make_op(name_,sym_) constexpr bool name_##OpLam = [] (const auto x,
const auto y) noexcept { return (x sym_ y);};
...
#define make_op(name_, sym_)                                         \
  ...
  struct name_##RevOp {                                              \
    ...
    static value_type eval(T x, T y) noexcept { return (y sym_ x); } \
            ...
            make_op(Add, +) make_op(Sub, -) make_op(Mul, *) make_op(Div, /)
                make_op(Xor, ^) make_op(Ior, |) make_op(And, &) make_op(Lsh, <<)
                    make_op(Rsh, >>)
#undef make_op
...
#define make_op(name_, sym_)                                         \
  ...
  struct name_##RevOp {                                              \
    ...
    static value_type eval(T x, T y) noexcept { return (y sym_ x); } \
                        ...
                        make_op(Eq, ==) make_op(Neq, !=) make_op(LtEq, <=)
                            make_op(GtEq, >=) make_op(Gt, >) make_op(Lt, <)
                                make_op(LogAnd, &&) make_op(LogOr, ||)
#undef make_op
