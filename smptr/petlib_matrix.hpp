...
template <typename T>
struct matrix : public array_base<matrix<T>, T> {
  typedef std::size_t size_type;
  typedef std::ptrdiff_t difference_type;
  typedef T* pointer;
  typedef const T* const_pointer;
  ...
  matrix(const matrix<T>& m)
      : data_(get_storage<T>(m.sz)), n1(m.n1), n2(m.n2), sz(m.sz) {
  ...
  matrix(matrix<T>&& m) : data_(m.data_), n1(m.n1), n2(m.n2), sz(m.sz) {}
    ...
    size_type extent1() const noexcept {
    return n1;}
    size_type extent2() const noexcept {
    return n2;}
    ...
    constexpr bool is_row_major() const noexcept {
    return true;}
    constexpr bool is_col_major() const noexcept {
    return false;}
    constexpr bool is_zero_offset() const noexcept {
    return true;}
...
petlib_make_op(+=) petlib_make_op(-=) petlib_make_op(*=) petlib_make_op(/=)
    petlib_make_op(^=) petlib_make_op(|=) petlib_make_op(&=) petlib_make_op(
        >>=) petlib_make_op(<<=)
#undef petlib_make_op
...
#define petlib_make_op(sym_)                                         \
  ...
  constexpr matrix<T>& operator sym_(const matrix_base<m_t, T>& m) { \
    ...
    return *this;                                                    \
        ...
        petlib_make_op(+=) petlib_make_op(-=) petlib_make_op(*=) petlib_make_op(
            /=) petlib_make_op(^=) petlib_make_op(|=) petlib_make_op(&=)
            petlib_make_op(>>=) petlib_make_op(<<=)
#undef petlib_make_op
...
#define petlib_make_op(name_, sym_)                                    \
  constexpr matrix<T>& operator sym_(const sub_matrix<T>& m) {         \
    ...
    return *this;                                                      \
                ...
                petlib_make_op(Add, +=) petlib_make_op(Sub, -=) petlib_make_op(
                    Mul, *=) petlib_make_op(Div, /=) petlib_make_op(Xor, ^=)
                    petlib_make_op(Ior, |=) petlib_make_op(And, &=)
                        petlib_make_op(Rsh, >>=) petlib_make_op(Lsh, <<=)
#undef petlib_make_op
...
#define petlib_make_op(name_, sym_)               \
  constexpr matrix<T>& operator sym_(const T x) { \
    ...
    return *this;                                 \
                            ...
                            petlib_make_op(Add, +=) petlib_make_op(Sub, -=)
                                petlib_make_op(Mul, *=) petlib_make_op(Div, /=)
                                    petlib_make_op(Xor, ^=)
                                        petlib_make_op(Ior, |=)
                                            petlib_make_op(And, &=)
                                                petlib_make_op(Rsh, >>=)
