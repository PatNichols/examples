...
struct array_ref {
  ...
  value_type operator[](size_type i) const noexcept { return a[i]; }
...
struct array_un_xpr {
  ...
  value_type operator[](size_type i) const noexcept { return Op::eval(a[i]); }
  size_type size() const noexcept { return a.size(); }
};
...
#define make_op(name_, sym_)                                                   \
  ...
  constexpr array_xpr<                                                         \
      array_bin_xpr<name_##Op<T>, array_ref<A, T>, array_ref<B, T> >, T>       \
  operator sym_(const array_base<A, T>& a,                                     \
                const array_base<B, T>& b) noexcept {                          \
    typedef array_bin_xpr<name_##Op<T>, array_ref<A, T>, array_ref<B, T> >     \
        xpr_t;                                                                 \
  ...
  template <class A, class B, typename T>                                      \
  constexpr array_xpr<                                                         \
      array_bin_xpr<name_##Op<T>, array_ref<A, T>, array_xpr<B, T> >, T>       \
  operator sym_(const array_base<A, T>& a,                                     \
                const array_xpr<B, T>& b) noexcept {                           \
    typedef array_bin_xpr<name_##Op<T>, array_ref<A, T>, array_xpr<B, T> >     \
        xpr_t;                                                                 \
  ...
  template <class A, class B, typename T>                                      \
  constexpr array_xpr<                                                         \
      array_bin_xpr<name_##Op<T>, array_xpr<A, T>, array_ref<B, T> >, T>       \
  operator sym_(const array_xpr<A, T>& a,                                      \
                const array_base<B, T>& b) noexcept {                          \
    typedef array_bin_xpr<name_##Op<T>, array_xpr<A, T>, array_ref<B, T> >     \
        xpr_t;                                                                 \
  ...
  template <class A, class B, typename T>                                      \
  constexpr array_xpr<                                                         \
      array_bin_xpr<name_##Op<T>, array_xpr<A, T>, array_xpr<B, T> >, T>       \
  operator sym_(const array_xpr<A, T>& a, const array_xpr<B, T>& b) noexcept { \
    typedef array_bin_xpr<name_##Op<T>, array_xpr<A, T>, array_xpr<B, T> >     \
        xpr_t;                                                                 \
  ...
  constexpr array_xpr<                                                         \
      array_bin_xpr<name_##Op<T>, array_ref<A, T>, scalar_ref<T> >, T>         \
  operator sym_(const array_base<A, T>& a, const T b) noexcept {               \
    typedef array_bin_xpr<name_##Op<T>, array_ref<A, T>, scalar_ref<T> >       \
        xpr_t;                                                                 \
  ...
  template <class A, typename T>                                               \
  constexpr array_xpr<                                                         \
      array_bin_xpr<name_##Op<T>, array_xpr<A, T>, scalar_ref<T> >, T>         \
  operator sym_(const array_xpr<A, T>& a, const T b) noexcept {                \
    typedef array_bin_xpr<name_##Op<T>, array_xpr<A, T>, scalar_ref<T> >       \
        xpr_t;                                                                 \
  ...
  template <class A, typename T>                                               \
  constexpr array_xpr<                                                         \
      array_bin_xpr<name_##RevOp<T>, array_ref<A, T>, scalar_ref<T> >, T>      \
  operator sym_(const T b, const array_base<A, T>& a) noexcept {               \
    typedef array_bin_xpr<name_##RevOp<T>, array_ref<A, T>, scalar_ref<T> >    \
        xpr_t;                                                                 \
    return array_xpr<xpr_t, T>(xpr_t(array_ref<A, T>(a), scalar_ref<T>(b)));   \
  }                                                                            \
                                                                               \
  template <class A, typename T>                                               \
  constexpr array_xpr<                                                         \
      array_bin_xpr<name_##RevOp<T>, array_xpr<A, T>, scalar_ref<T> >, T>      \
  operator sym_(const T b, const array_xpr<A, T>& a) noexcept {                \
    typedef array_bin_xpr<name_##RevOp<T>, array_xpr<A, T>, scalar_ref<T> >    \
        xpr_t;                                                                 \
    return array_xpr<xpr_t, T>(xpr_t(a, scalar_ref<T>(b)));                    \
  }
