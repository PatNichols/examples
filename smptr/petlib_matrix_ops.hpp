...
struct matrix_base {
  ...
  constexpr size_type extent1() const noexcept {
    return cast_down()->extent1();
  };
  constexpr size_type extent2() const noexcept {
    return cast_down()->extent2();
  };
...
#define make_op(name_, sym_)                                                   \
  ...
  constexpr matrix_xpr<                                                        \
      matrix_bin_xpr<name_##Op<T>, matrix_ref<A, T>, matrix_ref<B, T> >, T>    \
  operator sym_(const matrix_base<A, T>& a,                                    \
                const matrix_base<B, T>& b) noexcept {                         \
    typedef matrix_bin_xpr<name_##Op<T>, matrix_ref<A, T>, matrix_ref<B, T> >  \
        xpr_t;                                                                 \
    return matrix_xpr<xpr_t, T>(                                               \
        xpr_t(matrix_ref<A, T>(a), matrix_ref<B, T>(b)));                      \
  }                                                                            \
                                                                               \
  template <class A, class B, typename T>                                      \
  constexpr matrix_xpr<                                                        \
      matrix_bin_xpr<name_##Op<T>, matrix_ref<A, T>, matrix_xpr<B, T> >, T>    \
  operator sym_(const matrix_base<A, T>& a,                                    \
                const matrix_xpr<B, T>& b) noexcept {                          \
    typedef matrix_bin_xpr<name_##Op<T>, matrix_ref<A, T>, matrix_xpr<B, T> >  \
        xpr_t;                                                                 \
  ...
  template <class A, class B, typename T>                                      \
  constexpr matrix_xpr<                                                        \
      matrix_bin_xpr<name_##Op<T>, matrix_xpr<A, T>, matrix_ref<B, T> >, T>    \
  operator sym_(const matrix_xpr<A, T>& a,                                     \
                const matrix_base<B, T>& b) noexcept {                         \
    typedef matrix_bin_xpr<name_##Op<T>, matrix_xpr<A, T>, matrix_ref<B, T> >  \
        xpr_t;                                                                 \
  ...
  template <class A, class B, typename T>                                      \
  constexpr matrix_xpr<                                                        \
      matrix_bin_xpr<name_##Op<T>, matrix_xpr<A, T>, matrix_xpr<B, T> >, T>    \
  operator sym_(const matrix_xpr<A, T>& a,                                     \
                const matrix_xpr<B, T>& b) noexcept {                          \
    typedef matrix_bin_xpr<name_##Op<T>, matrix_xpr<A, T>, matrix_xpr<B, T> >  \
        xpr_t;                                                                 \
  ...
  constexpr matrix_xpr<                                                        \
      matrix_bin_xpr<name_##Op<T>, matrix_ref<A, T>, scalar_ref<T> >, T>       \
  operator sym_(const matrix_base<A, T>& a, const T b) noexcept {              \
    typedef matrix_bin_xpr<name_##Op<T>, matrix_ref<A, T>, scalar_ref<T> >     \
        xpr_t;                                                                 \
  ...
  template <class A, typename T>                                               \
  constexpr matrix_xpr<                                                        \
      matrix_bin_xpr<name_##Op<T>, matrix_xpr<A, T>, scalar_ref<T> >, T>       \
  operator sym_(const matrix_xpr<A, T>& a, const T b) noexcept {               \
    typedef matrix_bin_xpr<name_##Op<T>, matrix_xpr<A, T>, scalar_ref<T> >     \
        xpr_t;                                                                 \
  ...
  template <class A, typename T>                                               \
  constexpr matrix_xpr<                                                        \
      matrix_bin_xpr<name_##RevOp<T>, matrix_ref<A, T>, scalar_ref<T> >, T>    \
  operator sym_(const T b, const matrix_base<A, T>& a) noexcept {              \
    typedef matrix_bin_xpr<name_##RevOp<T>, matrix_ref<A, T>, scalar_ref<T> >  \
        xpr_t;                                                                 \
    return matrix_xpr<xpr_t, T>(xpr_t(matrix_ref<A, T>(a), scalar_ref<T>(b))); \
  }                                                                            \
                                                                               \
  template <class A, typename T>                                               \
  constexpr matrix_xpr<                                                        \
      matrix_bin_xpr<name_##RevOp<T>, matrix_xpr<A, T>, scalar_ref<T> >, T>    \
  operator sym_(const T b, const matrix_xpr<A, T>& a) noexcept {               \
    typedef matrix_bin_xpr<name_##RevOp<T>, matrix_xpr<A, T>, scalar_ref<T> >  \
        xpr_t;                                                                 \
    return matrix_xpr<xpr_t, T>(xpr_t(a, scalar_ref<T>(b)));                   \
  }
...
make_op(Add, +) make_op(Sub, -) make_op(Mul, *) make_op(Div, /) make_op(Xor, ^)
    make_op(Ior, |) make_op(And, &) make_op(Lsh, <<) make_op(Rsh, >>)
        make_op(Eq, ==) make_op(Neq, !=) make_op(LtEq, <=) make_op(GtEq, >=)
            make_op(Gt, >) make_op(Lt, <) make_op(LogAnd, &&) make_op(LogOr, ||)
#undef make_op
...
#define make_op(name_, sym_)                                              \
  ...
  operator sym_(const matrix_xpr<A, T>& a) {                              \
    ...
    return matrix_xpr<xpr_t, T>(xpr_t(a));                                \
                ...
                make_op(Com, ~) make_op(Neg, -) make_op(Pos, +) make_op(Not, !)
#undef make_op

                    template <class A, typename T>
                    std::ostream& operator<<(std::ostream& os,
                                             const matrix_base<A, T>& a) {
  ...
  if (csize < 6) {
    ...
    for (std::size_t i = 0; i < rsize; ++i) {
      for (std::size_t j = 0; j < csize; ++j) {
        os << std::setw(12) << i << " " << std::setw(12) << j << " " << a(i, j)
           << "\n";
      }
    }
    os << "----------------------------\n";
