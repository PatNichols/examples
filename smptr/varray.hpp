...
struct varray : public petlib::array_base<varray<T>, T> {
  ...
  varray(const array_base<v_t, T>& v)
      : data_(get_storage<T>(v.size())), sz(v.size()) {
...
#define PETLIB_MAKE_OPS_(sym_)                                        \
  ...
  PETLIB_MAKE_OPS(+=) 
  PETLIB_MAKE_OPS(-=)
                  PETLIB_MAKE_OPS(*=) PETLIB_MAKE_OPS(/=) PETLIB_MAKE_OPS(^=)
                      PETLIB_MAKE_OPS(|=) PETLIB_MAKE_OPS(&=)
                          PETLIB_MAKE_OPS(>>=) PETLIB_MAKE_OPS(<<=)
#undef PETLIB_MAKE_OPS
...
#define PETLIB_MAKE_OPS_(sym_)                                           \
  ...
  constexpr varray& operator sym_(const array_xpr<v_t, T>& v) noexcept { \
    for (size_type i = 0; i < sz; ++i) data_[i] sym_ v[i];
                                                          return *this;
} PETLIB_MAKE_OPS(=) PETLIB_MAKE_OPS(+=) PETLIB_MAKE_OPS(-=) PETLIB_MAKE_OPS(*=)
    PETLIB_MAKE_OPS(/=) PETLIB_MAKE_OPS(^=) PETLIB_MAKE_OPS(|=)
        PETLIB_MAKE_OPS(&=) PETLIB_MAKE_OPS(>>=) PETLIB_MAKE_OPS(<<=)
#undef PETLIB_MAKE_OPS
...
constexpr std::reverse_iterator<T> rbegin() noexcept {
  return reverse_iterator<T>(data_ + n);
}
constexpr const std::reverse_iterator<T> rbegin() const noexcept {
  return reverse_iterator<T>(data_ + n);
}
...
struct slice_array : public petlib::array_base<slice_array<T>, T> {
  ...
  explicit slice_array(pointer p, size_type n, size_type stride_)
      : data_(p), sz(n), str(stride_) {}

  slice_array(const slice_array<T>& v)
      : data_(v.data_), sz(v.sz), str(v, str) {}
...
#define PETLIB_MAKE_OPS_(sym_, name_)                                      \
  ...
  return *this;
} PETLIB_MAKE_OPS(+=, add_eq) PETLIB_MAKE_OPS(-=, sub_eq)
    PETLIB_MAKE_OPS(*=, mul_eq) PETLIB_MAKE_OPS(/=, div_eq)
        PETLIB_MAKE_OPS(^=, xor_eq) PETLIB_MAKE_OPS(|=, ior_eq)
            PETLIB_MAKE_OPS(&=, and_eq) PETLIB_MAKE_OPS(>>=, rsh_eq)
...
constexpr const iterator begin() const noexcept {
  return slice_iterator<T>(data_, str);
}
constexpr pointer end() noexcept { return data_ + n; }
constexpr const pointer end() const noexcept { return data_ + n; }
