...
struct slice {
  ...
  slice(size_type offset_, size_type length_, size_type stride_)
      : off_(offset_), len_(length_), str_(stride_) {}

  constexpr size_type offset() const noexcept { return off_; }
  constexpr size_type size() const noexcept { return len_; }
  constexpr size_type end_size() const noexcept {
    return (off_ + len_ * str_);
  };
...
template <typename T>
struct varray : public array_base<varray<T>, T> {
  ...
  varray(const varray& v) : data_(get_storage<T>(v.sz)), sz(v.sz) {
    copy(data_.get(), data_.get() + sz, v.data_.get());
  }

  varray(varray&& v) : data_(nullptr), sz(v.sz) {
    v.data_.swap(data_);
    v.sz = 0;
  }

  varray(std::initializer_list<T> ilist)
      : data_(get_storage<T>(ilist.size())), sz(ilist.size()) {
    copy(data_.get(), data_.get() + sz, ilist.begin());
  }

  varray(pointer p, size_type n) : data_(get_storage<T>(n)), sz(n) {
    copy(data_.get(), data_.get() + sz, p);
  }
  ...
  template <class A>
  varray(const array_base<A, T>& a)
      : data_(get_storage<T>(a.size())), sz(a.size()) {
    for (size_type i = 0; i < sz; ++i) data_[i] = a[i];
  }

  template <class A>
  varray(const array_xpr<A, T>& a)
      : data_(get_storage<T>(a.size())), sz(a.size()) {
    for (size_type i = 0; i < sz; ++i) data_[i] = a[i];
  }
  ...
  constexpr iterator end() noexcept { return iterator(data_.get() + sz); }
  ...
  constexpr pointer base() noexcept { return data_.get(); }
  constexpr const pointer base() const noexcept { return data_.get(); }

  constexpr varray<T> operator()(const range& r) const noexcept {
    return varray<T>(data_.get() + r.offset(), r.size());
  }
  constexpr varray<T> operator()(const slice& s) const noexcept {
    varray<T> v(s.size());
    slice_iterator<T> iter(data_.get() + s.offset(), s.stride());
    slice_iterator<T> iend(data_.get() + s.end_size(), s.stride());
    iterator riter(v.data_.get());
    while (iter != iend) {
      *riter = *iter;
      ++iter;
      ++riter;
  ...
  constexpr sub_array<T> operator()(const range& r) noexcept {
    return sub_array<T>(data_.get() + r.offset(), r.size());
  }
  constexpr slice_array<T> operator()(const slice& s) noexcept {
    return slice_array<T>(data_.get() + s.offset(), s.size(), s.stride());
  }
  ...
  void resize(size_type n) {
    ...
    if (data_) data_.reset();
    data_ = std::move(boost::shared_array<T>(get_storage<T>(n)));
    sz = n;
  }
};

template <typename T>
struct slice_array : public array_base<slice_array<T>, T> {
  typedef T value_type;
  typedef T& reference;
  typedef T* pointer;
  typedef std::size_t size_type;
  ...
  slice_array(slice_array&& v) : data_(v.data_), sz(v.sz), str(v.str) {
    v.data_.reset();
    v.sz = 0;
  }

  slice_array(pointer p, size_type n, size_type stride)
      : data_(p), sz(n), str(stride) {}
  ...
  constexpr iterator begin() noexcept { return iterator(data_.get(), str); }
  constexpr iterator end() noexcept {
    return iterator(data_.get() + sz * str, str);
  }
  constexpr reverse_iterator rbegin() noexcept {
    return reverse_iterator(this->end());
  }
  ...
  constexpr const_iterator begin() const noexcept {
    return const_iterator(data_.get(), str);
  }
  constexpr const_iterator end() const noexcept {
    return const_iterator(data_.get() + sz * str, str);
  }
  constexpr const_reverse_iterator rbegin() const noexcept {
    return reverse_iterator(this->end());
  }
  ...
  constexpr slice_array<T> operator()(const range& r) noexcept {
    return slice_array<T>(data_.get() + r.offset() * str, r.size(), str);
  }
  constexpr slice_array<T> operator()(const slice& sl) noexcept {
    return slice_array<T>(data_.get() + sl.offset() * str, sl.size(),
                          sl.stride() * str);
  };
  constexpr varray<T> operator()(const range& r) const noexcept {
    varray<T> v(r.size());
  ...
  constexpr varray<T> operator()(const slice& sl) const noexcept {
    varray<T> v(sl.size());
    iterator iter(data_.get() + sl.offset() * str, sl.stride() * str);
  ...
  constexpr varray<T> cshift(size_type n) const noexcept {
    varray<T> v(sz);
    iterator iter(data_.get() + n * str, str);
...
template <>
void varray<double>::random_fill() {
...
template <>
void varray<long double>::random_fill() {
  std::mt19937 rgen;
...
template <typename T>
struct sub_array : public array_base<sub_array<T>, T> {
  typedef T value_type;
  typedef T& reference;
  typedef T* pointer;
  typedef const T* const_pointer;
  ...
  constexpr varray<T> operator()(const slice& sl) const noexcept {
    varray<T> v(sl.size());
    slice_iterator<T> iter(data_.get() + sl.offset(), sl.stride());
    slice_iterator<T> iend(data_.get() + sl.end_size(), sl.stride());
    iterator riter(v.data_.get());
    while (iter != iend) {
      *riter = *iter;
      ++iter;
  ...
  constexpr slice_array<T> operator()(const slice& sl) noexcept {
    return slice_array<T>(data_.get() + sl.offset(), sl.size(), sl.stride());
  }
  ...
  void elim(const sub_array<T>& v) {
    ...
    T nrmp = norm2<T>(v.begin(), v.end());
    T fact = dotp / nrmp;
    for (size_type i = 0; i < sz; ++i) data_.get()[i] -= fact * v[i];
  }
