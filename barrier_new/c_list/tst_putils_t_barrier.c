...
void * sum_fun(void *args)
{
    ...
    size_t n = sum_t->n;
    putils_tour_barrier_t *b = sum_t->barr;
    ...
    psum[id]=sum;
    putils_tour_barrier_reduce(b,id,&sum_red,(void*)&(sum_t->sinfo));
    sum = 0.0;
    for (i=my_off;i<off;++i) {
        sum += arr[i];
    }
...
double summ(size_t n,size_t nth)
{
    ...
    b=putils_tour_barrier_init(nth);  
    srand(time(0));
