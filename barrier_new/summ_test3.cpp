...
struct summer {
  ...
  void sum_it() {
    ...
    thds[0] =
        new std::thread(do_sum2<barrier_t>, reinterpret_cast<void*>(infos[0]));
    for (std::size_t i = 1; i < nth; ++i) {
      infos[i] = new sum_info_t<barrier_t>(*infos[0]);
      infos[i]->set_id(i);
      thds[i] =
          new std::thread(do_sum2<barrier_t>,reinterpret_cast<void*>(infos[i])); 
    }
    std::size_t j = nth;
    for (std::size_t i = 0; i < nth; ++i) {
      --j;
...
int main() {
  ...
  for (auto n : nsize) {
    for (auto nth : nthds) {
      test_it<putils::spinlock_barrier>(n, nth);
    }
  }
  return EXIT_SUCCESS;
}
